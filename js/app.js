...
$(window).scroll(function(){
    ...
    if(wScroll >= $('#contact').offset().top-500){
        $('.contact').each(function(i){
            setTimeout(function(){
                $('.contact').eq(i).show('scale');
                // console.log('anjay')
            },500*(i+1))
        })
    }
    if(wScroll >= $('#recent').offset().top-400){
    ...
    if(wScroll >= $('#recent').offset().top-700){
...
$('form').on('submit', function(e){
    ...
    $('.send').attr('href',`mailto:naninunet46@gmail.com?subject=${inputSubject}&body=${inputMessage}`)
})
$('#message').on('keyup',function(){
        ...
        $('.send').attr('href',`mailto:naninunet46@gmail.com?subject=${inputSubject}&body=${inputMessage}%20`)
        console.log($('.send'))
})